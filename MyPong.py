...
def updatePaddle1(action, paddle1YPos,dft):
	...
	if (paddle1YPos < 0):
		paddle1YPos = 0
	if (paddle1YPos > GAME_HEIGHT - PADDLE_HEIGHT):
		paddle1YPos = GAME_HEIGHT - PADDLE_HEIGHT
	return paddle1YPos
...
def updatePaddle2(paddle2YPos, ballYPos,dft):
	...
	if (paddle2YPos > GAME_HEIGHT - PADDLE_HEIGHT):
		paddle2YPos = GAME_HEIGHT - PADDLE_HEIGHT
	return paddle2YPos
...
class PongGame:
	...
	def PlayNextMove(self, action):
		...
		if(score >0.5 or score < -0.5):
			self.GScore = 0.1*score + self.GScore*0.9 
